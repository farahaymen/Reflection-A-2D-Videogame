...
public class L1Cs4Manager : MonoBehaviour
{
    [Header("Animation-Related")]
    public Animator screenAnim;
    public Animator steveAnim;
    ...
    void Awake() 
    {
        ...
        if (steveAnim == null)
            steveAnim = GameObject.Find("steve").GetComponent<Animator>();

        AudioManager.instance.setSfx(rainSfx);
    ...
    void Update() 
    {
        ...
        if (dm.dialogueDone && prevDialoguesDone == 2)
        {
            dm.dialogueDone = false;
            prevDialoguesDone++;
            steveAnim.SetBool("isCrying", true);
        ...
        if (dm.dialogueDone && prevDialoguesDone == 3)
        {
            ...
            dm.dialogueDone = false;
            prevDialoguesDone++;
            Invoke("NextScene", 3.0f); //goes to next level's cutscene after 5 seconds
        }
    }
